...
class DBManager:
    ...
    # --- Tasks CRUD ---
    def create_task(self, description: str, expected_output: str, agent_id: Optional[int], tools: List[str] = None, required_inputs: List[Dict[str, str]] = None, vector_dbs: List[str] = None, agent_specialization: Optional[str] = None, name: Optional[str] = None, model_id: Optional[int] = None, human_validation: bool = False, max_input_context: int = 0, max_output_tokens: int = 0) -> int:
        tools = tools or []
        ...
        sql = "INSERT INTO tasks (description, expected_output, agent_id, tools_json, required_inputs_json, vector_dbs_json, agent_specialization, name, model_id, human_validation, max_input_context, max_output_tokens) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        self.cursor.execute(sql, (description, expected_output, agent_id, tools_json, required_inputs_json, vector_dbs_json, agent_specialization or None, name or None, model_id, int(human_validation), max_input_context, max_output_tokens))
        self.conn.commit()
        return self.cursor.lastrowid
    ...
    def update_task(self, task_id: int, description: str, expected_output: str, agent_id: Optional[int], tools: List[str] = None, required_inputs: List[Dict[str, str]] = None, vector_dbs: List[str] = None, agent_specialization: Optional[str] = None, name: Optional[str] = None, model_id: Optional[int] = None, human_validation: bool = False, max_input_context: int = 0, max_output_tokens: int = 0) -> int:
        ...
        sql = """
        UPDATE tasks 
        SET description = ?, expected_output = ?, agent_id = ?, tools_json = ?, required_inputs_json = ?, vector_dbs_json = ?, agent_specialization = ?, name = ?, model_id = ?, human_validation = ?, max_input_context = ?, max_output_tokens = ?
        WHERE id = ?
        """
        self.cursor.execute(sql, (description, expected_output, agent_id, tools_json, required_inputs_json, vector_dbs_json, agent_specialization or None, name or None, model_id, int(human_validation), max_input_context, max_output_tokens, task_id))
        self.conn.commit()
        return self.cursor.rowcount
