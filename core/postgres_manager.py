...
class PostgresManager:
    ...
    # --- Helper Methods ---
    def _to_dict(self, row: dict) -> Optional[Dict[str, Any]]:
        """Converts a dict object to a dictionary."""
    ...
    def create_vector_db(self, name: str, path: str, provider: str, model_name: str) -> int:
        ...
        sql = "INSERT INTO vector_databases (name, path, provider, model_name) VALUES (%s, %s, %s, %s) RETURNING id"
    ...
    def delete_vector_db(self, db_id: int) -> int:
        sql = "DELETE FROM vector_databases WHERE id = %s"
    ...
    def create_model(self, provider: str, model_name: str, env_var_name: str = "", is_local: bool = False, vram_gb: float = 0.0, supports_tools: bool = True) -> int:
        sql = "INSERT INTO models (provider, model_name, env_var_name, is_local, vram_gb, supports_tools) VALUES (%s, %s, %s, %s, %s, %s) RETURNING id"
        self.cursor.execute(sql, (provider, model_name, env_var_name, bool(is_local), vram_gb, bool(supports_tools)))
        self.conn.commit()
        return self.cursor.fetchone()['id']
    ...
    def read_model_by_name(self, model_name: str) -> Optional[Dict[str, Any]]:
        sql = "SELECT * FROM models WHERE model_name = %s LIMIT 1"
    ...
    def update_model(self, model_id: int, provider: str, model_name: str, env_var_name: str = "", is_local: bool = False, vram_gb: float = 0.0, supports_tools: bool = True) -> int:
        sql = "UPDATE models SET provider = %s, model_name = %s, env_var_name = %s, is_local = %s, vram_gb = %s, supports_tools = %s WHERE id = %s"
        self.cursor.execute(sql, (provider, model_name, env_var_name, bool(is_local), vram_gb, bool(supports_tools), model_id))
        self.conn.commit()
        return self.cursor.rowcount
    ...
    def create_agent(self, name: str, role: str, backstory: str, model_id: Optional[int], tools: List[str]) -> int:
        ...
        sql = "INSERT INTO agents (name, role, backstory, model_id, tools_json) VALUES (%s, %s, %s, %s, %s) RETURNING id"
    ...
    def update_agent(self, agent_id: int, name: str, role: str, backstory: str, model_id: Optional[int], tools: List[str]) -> int:
        ...
        sql = """
        UPDATE agents 
        SET name = %s, role = %s, backstory = %s, model_id = %s, tools_json = %s 
        WHERE id = %s
        """
    ...
    def create_task(self, description: str, expected_output: str, agent_id: Optional[int], tools: List[str] = None, required_inputs: List[Dict[str, str]] = None, vector_dbs: List[str] = None, agent_specialization: Optional[str] = None, name: Optional[str] = None, model_id: Optional[int] = None, human_validation: bool = False, max_input_context: int = 0, max_output_tokens: int = 0, output_pydantic: Optional[str] = None, tool_profile: Optional[str] = None) -> int:
        ...
        sql = "INSERT INTO tasks (description, expected_output, agent_id, tools_json, required_inputs_json, vector_dbs_json, agent_specialization, name, model_id, human_validation, max_input_context, max_output_tokens, output_pydantic, tool_profile) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id"
    ...
    def update_task(self, task_id: int, description: str, expected_output: str, agent_id: Optional[int], tools: List[str] = None, required_inputs: List[Dict[str, str]] = None, vector_dbs: List[str] = None, agent_specialization: Optional[str] = None, name: Optional[str] = None, model_id: Optional[int] = None, human_validation: bool = False, max_input_context: int = 0, max_output_tokens: int = 0, output_pydantic: Optional[str] = None, tool_profile: Optional[str] = None) -> int:
        ...
        sql = """
        UPDATE tasks 
        SET description = %s, expected_output = %s, agent_id = %s, tools_json = %s, required_inputs_json = %s, vector_dbs_json = %s, agent_specialization = %s, name = %s, model_id = %s, human_validation = %s, max_input_context = %s, max_output_tokens = %s, output_pydantic = %s, tool_profile = %s
        WHERE id = %s
        """
    ...
    def delete_task(self, task_id: int) -> int:
        ...
        for wf in workflows:
            ...
            if any(get_tid(tid) == task_id for tid in task_ids):
                ...
                self.cursor.execute(
                    "UPDATE workflows SET task_ids_json = %s, has_deletion_warning = 1 WHERE id = %s",
        ...
        if remaining_tasks:
            ...
            self.cursor.execute("SELECT column_name as name FROM information_schema.columns WHERE table_name = 'tasks'")
            ...
            for wf in current_workflows:
                try:
                    ...
                    self.cursor.execute("UPDATE workflows SET task_ids_json = %s WHERE id = %s", 
                                        (json.dumps(new_t_ids), wf['id']))
                except (json.JSONDecodeError, TypeError):
                    continue
            
            # Reset SQLite autoincrement sequence
            self.cursor.execute("SELECT setval(pg_get_serial_sequence('tasks', 'id'), COALESCE((SELECT MAX(id) FROM tasks), 1))", (len(all_task_data),))
        else:
            # If no tasks left, reset sequence
            self.cursor.execute("SELECT setval(pg_get_serial_sequence('tasks', 'id'), 1, false)")
    ...
    def dismiss_workflow_warning(self, workflow_id: int) -> None:
        ...
        self.cursor.execute("UPDATE workflows SET has_deletion_warning = 0 WHERE id = %s", (workflow_id,))
        self.conn.commit()
    ...
    def delete_workflow(self, workflow_id: int) -> int:
        ...
        self.cursor.execute("DELETE FROM workflows WHERE id = %s", (workflow_id,))
        ...
        if remaining:
            ...
            self.cursor.execute("SELECT column_name as name FROM information_schema.columns WHERE table_name = 'workflows'")
            columns = [col['name'] for col in self.cursor.fetchall()]
            cols_str = ", ".join(columns)
            placeholders = ", ".join(["%s"] * len(columns))
            ...
            self.cursor.execute("SELECT setval(pg_get_serial_sequence('workflows', 'id'), COALESCE((SELECT MAX(id) FROM workflows), 1))", (len(all_wf_data),))
        else:
            self.cursor.execute("SELECT setval(pg_get_serial_sequence('workflows', 'id'), 1, false)")
    ...
    def get_context(self, session_id: str) -> Optional[Dict[str, Any]]:
        sql = "SELECT * FROM context_memory WHERE session_id = %s"
        self.cursor.execute(sql, (session_id,))
        row = self.cursor.fetchone()
        return self._to_dict(row)

    def clear_context(self, session_id: str) -> int:
        sql = "DELETE FROM context_memory WHERE session_id = %s"
        self.cursor.execute(sql, (session_id,))
        self.conn.commit()
        return self.cursor.rowcount
    ...
    def create_run(self, workflow_id: int, status: str = 'running', inputs: dict = None, source: str = 'telegram') -> int:
        ...
        sql = "INSERT INTO workflow_runs (workflow_id, status, current_task_idx, task_outputs, inputs, source, in_flight_tasks) VALUES (%s, %s, 0, '{}', %s, %s, '[]') RETURNING id"
    ...
    def read_all_runs(self, limit: int = 20) -> List[Dict[str, Any]]:
        sql = "SELECT * FROM workflow_runs ORDER BY started_at DESC LIMIT %s"
    ...
    def delete_run(self, run_id: int) -> int:
        ...
        sql = "DELETE FROM workflow_runs WHERE id = %s"
        self.cursor.execute(sql, (run_id,))
        self.conn.commit()
        return self.cursor.rowcount
    ...
    def set_hitl_answer(self, chat_id: str, answer: str) -> bool:
        sql = "UPDATE hitl_requests SET answer = %s, status = 'replied', updated_at = CURRENT_TIMESTAMP WHERE chat_id = %s"
    ...
    def get_app(self, app_id: int) -> Optional[Dict[str, Any]]:
        ...
        sql = "SELECT * FROM apps WHERE id = %s"
    ...
    def get_app_by_name(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        sql = "SELECT * FROM apps WHERE name = %s"
    ...
    def get_app_by_api_key(self, api_key: str) -> Optional[Dict[str, Any]]:
        ...
        sql = "SELECT * FROM apps WHERE api_key = %s"
    ...
    def update_app(self, app_id: int, **kwargs) -> int:
        ...
        sql = f"UPDATE apps SET {', '.join(set_clauses)} WHERE id = %s"
    ...
    def delete_app(self, app_id: int) -> int:
        ...
        self.cursor.execute("UPDATE workflows SET app_id = NULL WHERE app_id = %s", (app_id,))
        sql = "DELETE FROM apps WHERE id = %s"
        self.cursor.execute(sql, (app_id,))
        self.conn.commit()
        return self.cursor.rowcount
    ...
    def regenerate_app_api_key(self, app_id: int) -> str:
        ...
        self.cursor.execute("UPDATE apps SET api_key = %s WHERE id = %s", (new_key, app_id))
        self.conn.commit()
        return new_key

    def get_app_workflows(self, app_id: int) -> List[Dict[str, Any]]:
        """Returns all workflows linked to a specific app."""
        sql = "SELECT * FROM workflows WHERE app_id = %s ORDER BY name"
        self.cursor.execute(sql, (app_id,))
        rows = self.cursor.fetchall()
        processed = []
        for row in rows:
    ...
    def get_app_runs(self, app_id: int, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        WHERE w.app_id = %s AND wr.source = 'api'
